...
namespace Celeste.Mod.TetraHelper.Entities
{
    ...
    public class DashMatchWallRenderer : Entity
    {
        ...
        private List<DashMatchWall> list = new List<DashMatchWall>();
        ...
        {
            self.Level.Add(new DashMatchWallRenderer());
            orig(self);
        }


        public void Track(DashMatchWall block)
        {
            list.Add(block);
            if (tiles == null)
            {
        ...
        public void Untrack(DashMatchWall block)
        {
            list.Remove(block);
            if (list.Count <= 0)
            {
